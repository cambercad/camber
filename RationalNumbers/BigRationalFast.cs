...
// Vendored from Christian Ohle's BigRat (RationalNumerics), MIT License.
// https://github.com/c-ohle/RationalNumerics
// Currently optional: enable via #define FAST_BIG_RAT in BigRationalHybrid.cs.
// See third_party/MIT-Christian-Ohle-BigRat.txt.
...
namespace GeoCore
{
      ...
      IEquatable<BigRat>, IComparable<BigRat>, IComparable, IFormattable, ISpanFormattable
    {
        ...
        public static bool TryParse(string s, out BigRat result)
        {
        ...
        public static bool TryParse(string s, NumberStyles style, IFormatProvider provider, out BigRat result)
        {
        ...
        public static bool TryParse(string s, IFormatProvider provider, out BigRat result)
        {
        ...
        //    // if ((na < nb ? na : nb) >= 32 && na + nb < 25000) { kmu(a, na, b, nb, r); ... } // needs kmu

        //    if (na == 1 && nb == 1)
        //    {
        //        *(ulong*)r = (ulong)a[0] * b[0];
        //        return r[1] == 0 ? 1u : 2u;
        //    }
        ...
        //    {
        //        ulong carry = 0, acc;
        //        for (uint k = 0; k < na; k++, carry = acc >> 32)
        //            r[k] = unchecked((uint)(acc = carry + (ulong)a[k] * b[0]));
        //        r[na] = unchecked((uint)carry);
        //        uint len = na + 1;
        ...
        //    {
        //        ulong blo = nb == 2 ? *(ulong*)b : b[0];
        //        *(ulong*)(r + 2) = Bmi2.X64.MultiplyNoFlags(*(ulong*)a, blo, (ulong*)r);
        //        uint len = 4;
        //        while (len > 1 && r[len - 1] == 0)
        //            len--;
        //        return len;
        ...
        //    for (uint k = 0; k < na; k++, c = d >> 32)
        //        r[k] = unchecked((uint)(d = c + (ulong)a[k] * b[0]));
        //    r[na] = unchecked((uint)c);
        //    for (uint i = 1; i < nb; i++)
        //    {
        //        c = 0;
        //        for (uint k = 0; k < na; k++, c = d >> 32)
        //            r[i + k] = unchecked((uint)(d = r[i + k] + c + (ulong)a[k] * b[i]));
        //        r[i + na] = unchecked((uint)c);
        //    }
