...
namespace NURBS
{
  ...
  using cInt = Int32;
    ...
    {
        public List<PolyNode> m_AllPolys = new List<PolyNode>();
    ...
    //{
    //    private static readonly Int128 zero = new Int128(0);
    ...
    //        return (val1.high == val2.high && val1.low == val2.low);
    //    }
    ...
    //        return (i128.high == high && i128.low == low);
    //    }
    ...
    //    {
    //        if (val.low == 0)
    //            return new Int128(-val.high, 0);
    //        else
    //            return new Int128(~val.high, ~val.low + 1);
    //    }
    ...
    //        {
    //            if (val.low == 0)
    //                return (double)val.high * shift64;
    //            else
    //                return -(double)(~val.low + ~val.high * shift64);
    //        }
    //        else
    //            return (double)(val.low + val.high * shift64);
    //    }
    ...
    public const cInt hiRange = 0x7FFF;
        ...
        private static Int128 Int128Mul(long lhs, long rhs)
        {
            return (Int128)lhs * (Int128)rhs;
        }
    ...
    public class ClipperOffset
    {
        ...
        IntPoint OffsetFromSource(int j, double dx, double dy)
        {
            return new IntPoint(
                Round(m_srcPoly[j].X + dx),
                Round(m_srcPoly[j].Y + dy),
                m_srcPoly[j].Z);
        }
        //------------------------------------------------------------------------------

        /// <summary>
        /// Keep source-vertex Z on offset intersections: prefer a Z shared by both
        /// edges (the concave miter of a tagged corner), else a Z shared by both
        /// ends of one edge (join/cap arc).
        /// </summary>
        internal static void FillOffsetZ(
            IntPoint bot1, IntPoint top1, IntPoint bot2, IntPoint top2, ref IntPoint pt)
        {
            if (pt.Z != 0)
                return;
            cInt a1 = bot1.Z, a2 = top1.Z, b1 = bot2.Z, b2 = top2.Z;
            if (a1 != 0 && (a1 == b1 || a1 == b2)) { pt.Z = a1; return; }
            if (a2 != 0 && (a2 == b1 || a2 == b2)) { pt.Z = a2; return; }
            if (a1 != 0 && a1 == a2) { pt.Z = a1; return; }
            if (b1 != 0 && b1 == b2) { pt.Z = b1; return; }
        }
        //------------------------------------------------------------------------------
            ...
            for (int i = 0; i < m_polyNodes.ChildCount; i++)
            {
                ...
                if (len == 1)
                {
                    if (node.m_jointype == JoinType.jtRound)
                    {
                        ...
                        {
                            m_destPoly.Add(OffsetFromSource(0, X * delta, Y * delta));
                            double X2 = X;
                            X = X * m_cos - m_sin * Y;
                            Y = X2 * m_sin + Y * m_cos;
                        }
                ...
                else
                {
                    ...
                    {
                        int j = len - 1;
                        pt1 = OffsetFromSource(j, m_normals[j].X * delta, m_normals[j].Y * delta);
                        m_destPoly.Add(pt1);
                        pt1 = OffsetFromSource(j, -m_normals[j].X * delta, -m_normals[j].Y * delta);
                        m_destPoly.Add(pt1);
                    }
                    ...
                    if (node.m_endtype == EndType.etOpenButt)
                    {
                        pt1 = OffsetFromSource(0, -m_normals[0].X * delta, -m_normals[0].Y * delta);
                        m_destPoly.Add(pt1);
                        pt1 = OffsetFromSource(0, m_normals[0].X * delta, m_normals[0].Y * delta);
                        m_destPoly.Add(pt1);
                    }
        ...
        public void Execute(ref Paths solution, double delta)
        {
            ...
            Clipper clpr = new Clipper(Clipper.ioPreserveCollinear);
            clpr.ZFillFunction = FillOffsetZ;
        ...
        void OffsetPoint(int j, ref int k, JoinType jointype)
        {
            ...
            if (Math.Abs(m_sinA * m_delta) < 1.0)
            {
                ...
                {
                    m_destPoly.Add(OffsetFromSource(j, m_normals[k].X * m_delta, m_normals[k].Y * m_delta));
                    return;
                }
            ...
            if (m_sinA * m_delta < 0)
            {
                ...
                m_destPoly.Add(OffsetFromSource(j, m_normals[j].X * m_delta, m_normals[j].Y * m_delta));
            }
            else
                switch (jointype)
                {
        ...
        internal void DoSquare(int j, int k)
        {
            ...
            m_destPoly.Add(OffsetFromSource(j,
        ...
        internal void DoMiter(int j, int k, double r)
        {
            double q = m_delta / r;
            m_destPoly.Add(OffsetFromSource(j,
                (m_normals[k].X + m_normals[j].X) * q,
                (m_normals[k].Y + m_normals[j].Y) * q));
        }
        //------------------------------------------------------------------------------
        ...
        internal void DoRound(int j, int k)
        {
            ...
            {
                m_destPoly.Add(OffsetFromSource(j, X * m_delta, Y * m_delta));
                X2 = X;
                X = X * m_cos - m_sin * Y;
                Y = X2 * m_sin + Y * m_cos;
            }
